...
class WeChatService {
    ...
    async checkAuthWithCookies(userCookies) {
    ...
    async extractAuthFromResponse(response) {
        ...
        if (!tokenMatch)
            tokenMatch = html.match(/window\.__TOKEN__\s*=\s*["']([^"']+)["']/);
        // Pattern 8: look for token in any script tag
        if (!tokenMatch)
            tokenMatch = html.match(/<script[^>]*>[^<]*token["']?\s*:\s*["']([^"']+)["']/);
        if (!tokenMatch) {
